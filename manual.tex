...
\begin{document}
    ...
    \section{Uvedení do problematiky}
		...
		\subsection{IMF - Internet message format}
			...
			\texttt{CR} (ASCII hodnota 13) a~\texttt{LF} (ASCII hodnota 10) \cite{web:imf}.
		...
		\subsection{POP3 pøíkazy}
			...
			\begin{itemize}
				\item \textsc{USER} jmeno\_uzivatele
				\item \textsc{PASS} heslo
				\item \textsc{APOP} jmeno\_uzivatele heslo
				\item \textsc{STAT}
				\item \textsc{LIST} [message\_id]
				\item \textsc{RETR} message\_id
				\item \textsc{DELE} message\_id
				\item \textsc{NOOP}
				\item \textsc{RSET}
				\item \textsc{QUIT}
				\item \textsc{TOP} message\_id lines
				\item \textsc{UIDL} [message\_id]
			\end{itemize}
			Pro pøihlá¹ení na server slou¾í pøíkazy \textsc{USER}, \textsc{PASS} a~\textsc{APOP}. \textsc{APOP} vyu¾ívá
			...
			\textsc{USER} a~\textsc{PASS} slou¾í k~ne¹ifrovanému pøenosu. Pokud server podporuje \textsc{APOP},
			nesmí podporovat pøíkazy \textsc{USER} ani \textsc{PASS} a~obrácenì. V~programu popser lze
			pøepínat mezi podporou tìchto pøíkazù pomocí argumentù programu pøi jeho
			spu¹tìní \cite{web:pop3}.

			K~pøeètení zprávy lze pou¾ít pøíkazy \textsc{TOP} a~\textsc{RETR}. Pomocí pøíkazy \textsc{TOP}
			...
			K~získání identifikátorù emailù lze pou¾ít pøíkazi \textsc{LIST} a~\textsc{UIDL}. List
			vypí¹e v¹echny emaily, které nejsou oznaèeny ke smazaní a~ka¾dému pøidìlí
			\textsc{ID} a~vedle \textsc{ID} vypí¹e jeho velikost v~oktetech. \textsc{UIDL} k~\textsc{ID} zprávám vypí¹e
			jejich unikátní \textsc{ID} v~rámci celého Maildiru. \textsc{ID} zpráv se na rozdíl od
			tohoto unikátního \textsc{ID} mù¾e mìnit (\textsc{ID} je unikátní pouze v rámci jednoho
			sezení) \cite{web:pop3}.

			Pro zji¹tìní velikosti Maildiru (bez slo¾ky tmp) lze pou¾ít pøíkaz \textsc{STAT}.
			Uvedená velikost je v~oktetech \cite{web:pop3}.

			\textsc{DELE} slou¾í k~mazání zpráv. Zprávy se ma¾ou a¾ po zadání pøíkazu \textsc{QUIT}.
			Do té doby je mo¾né obnovit zprávy oznaèené ke smazání pøíkazem \textsc{RSET} \cite{web:pop3}.

			Na pøíkaz \textsc{NOOP} server pouze odpoví \cite{web:pop3}.
		...
		\subsection{stavy serveru}
			...
			\begin{itemize}
				...
				\item UPDATE
			\end{itemize}
			Ve stavu AUTHORIZATION lze pouze pou¾ívát pøíkazy \textsc{USER}, \textsc{PASS}, \textsc{APOP} a~\textsc{QUIT},
			...
			TRANSACTION lze pou¾ít v¹echny pøíkazy, vyjma \textsc{USER}, \textsc{PASS} a~\textsc{APOP}. Stav
			TRANSACTION trvá doku klient nezadá \textsc{QUIT}, poté se pøejde do stavu UPDATE.
    ...
    \section{Návrh programu}
		...
		Následnì program oèekává pøíkaz \textsc{APOP} nebo \textsc{USER} a~\textsc{PASS}, podle argumentù programu.
		...
		Pokud klient zadá pøíkaz \textsc{QUIT}, server sma¾e v¹echny soubory oznaèené ke smazání.
		Pokud se klient odpojí bez u¾ití pøíkazu \textsc{QUIT} nebo se spojení pøeru¹í,
	...
	\section{Návod k pou¾ití}
		...
		\subsection{Spu¹tìní serveru}
			...
			AUTH\_FILE je cesta k souboru s u¾ivatelským jménem a heslem. MAILDIR
			je cesta ke slo¾ce, která obsahuje minimálnì slo¾ky new a cur. S touto
			slo¾kou bude následnì popser pracovat. PORT je èíslo portu, na kterém
			se budou moci pøipojit klienti. Pokud je zadám parametr -c, tak se
			se pøihla¹ovací údaje zadávají pomocí \textsc{USER} a \textsc{PASS} a heslo je na síti
		...
		\subsection{Pøíklady}
			...
			autorizovat se bude muset u¾ivatel pomocí pøíkazu \textsc{APOP}.
