...
\begin{document}
    ...
    \section{Uvedení do problematiky}
		...
		\subsection{IMF - Internet message format}
			...
			mohou nabývat US-ASCII hodnot 1-127.\ref{imf}

			Zprávu tvoøí jednotlivé øádky. Ka¾dý øádek je ukonèena dvìmi po sobì jdoucími znaky -
			CR (ASCII hodnota 13) a LF (ASCII hodnota 10).\ref{imf}
			...
			Tìlo je opìt sekvence øádkù.\ref{imf}
		...
		\subsection{POP3 pøíkazy}
			...
			pokud je zadané vìt¹í èíslo, ne¾ je celkový poèet øádkù, ode¹le se celý email.\ref{pop3}
			...
			tohoto unikátního ID mù¾e mìnit.\ref{pop3}

			Pro zji¹tìní velikosti Maildiru (bez slo¾ky tmp) lze pou¾ít pøíkaz STAT.
			Uvedená velikost je v oktetech.\ref{pop3}

			DELE slou¾í k mazání zpráv. Zprávy se ma¾ou a¾ po zadání pøíkazu QUIT.
			Do té doby je mo¾né obnovit zprávy oznaèené ke smazání pøíkazem RSET.\ref{pop3}

			Na pøíkaz NOOP server pouze odpoví.\ref{pop3}
		...
		\subsection{stavy serveru}
			...
			Ve stavu UPDATE se provede mazání zpráv a následné ukonèení komunikace.\ref{pop3}
